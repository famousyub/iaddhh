...
public interface ProductsRepository extends JpaRepository<Product, Long> {
    ...
    @Query("select p from Product p left join fetch p.tags t left join fetch p.categories c left join fetch p.images i " +
            "where p.id = :id")
    Optional<Product> findByIdWithTagsCategoriesAndImages(Long id);
